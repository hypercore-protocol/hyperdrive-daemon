...
const Nanoresource = require('nanoresource-promise/emitter')
...
class DriveManager extends Nanoresource {
  constructor (corestore, network, db, opts = {}) {
    super()
    ...
    const dbs = DriveManager.SUB_DBS.reduce((acc, dbInfo) => {
      acc[dbInfo.prefix] = sub(this.db, dbInfo.prefix, { valueEncoding: dbInfo.valueEncoding })
    })

    this._driveIndex = dbs.drives
    this._mirrorIndex = dbs.mirroring
  ...
  async get (key, opts = {}) {
    ...
    const mountListener = async (trie) => {
      ...
      const parentConfig = (await this.network.getConfiguration(drive.discoveryKey)) || initialConfig || {}
      const existingMountConfig = (await this.network.getConfiguration(feed.discoveryKey)) || {}
      const mountConfig = {
      ...
      if (mountConfig) await this.network.configure(feed.discoveryKey, mountConfig)
  ...
  async _rpcAllNetworkConfigurations (call) {
    return this.network._rpcAllConfigurations(call)
    const networkConfigurations = await this.network.getAllConfigurations()
  ...
  async _rpcConfigureNetwork (call) {
    ...
    await this.network.configure(drive.metadata.discoveryKey, { ...opts })
