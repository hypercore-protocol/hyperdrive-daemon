...
class DriveManager extends EventEmitter {
  constructor (cores, networking, db, opts = {}) {
    super()
  ...
  async _remirror () {
    const mirrorList = await collect(this._mirrorIndex)
    for (const { key } of mirrorList) {
      const drive = await this.get({ key })
      await this.configureDrive(drive, { sparse: false })
    }
  }
  ...
  async _startMirroring (drive) {
    ...
    const mounts = await new Promise((resolve, reject) => {
      drive.getAllMounts({ memory: false, recursive: true }, (err, mounts) => {
    ...
    this._mirrorSessions.set(driveKey, mirrorSession)

    for (const [, { metadata, content }] of mounts) {
      const metadataKey = metadata.key.toString('hex')
      const contentKey = content.key.toString('hex')
      if (!this._mirrorRanges.has(metadataKey)) {
        const range = metadata.download()
        this._mirrorRanges.set(metadataKey, { core: metadata, range })
      }
      if (!this._mirrorRanges.has(contentKey)) {
        const range = content.download()
        this._mirrorRanges.set(contentKey, { core: content, range })
      }
    }
  }
  ...
  async _stopMirroring (drive) {
    ...
    for (const [, { metadata, content }] of mounts) {
      ...
      const contentKey = content.key.toString('hex')
      if (this._mirrorRanges.has(metadataKey)) {
        const { core, range } = this._mirrorRanges.get(metadataKey)
        core.undownload(range)
        this._mirrorRanges.delete(metadataKey)
      }
      if (this._mirrorRanges.has(contentKey)) {
        const { core, range } = this._mirrorRanges.get(contentKey)
        core.undownload(range)
        this._mirrorRanges.delete(contentKey)
      }
    }
    if (this._mirrorSessions.has(driveKey)) {
      await this.closeSession(this._mirrorSessions.get(driveKey))
    }
  }
  ...
  async get (key, opts = {}) {
    ...
    if (!drive) {
      ...
      drive = hyperdrive(this.cores.corestore, key, {
  ...
  async configureDrive (drive, opts = {}) {
  ...
  async _rpcConfigure (call) {
    const id = call.request.getId()

    if (!id) throw new Error('A configuration request must specify a session ID.')
    const drive = this.driveForSession(id)
    const opts = fromDriveConfiguration(call.request.getConfig())

    await this.configureDrive(drive, { ...opts })

    const rsp = new rpc.drive.messages.ConfigureDriveResponse()
    return rsp
  }
