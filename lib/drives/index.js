...
class DriveManager extends EventEmitter {
  ...
  async _rejoin () {
    ...
    for (const { key: discoveryKey, value: networkOpts } of driveList) {
      ...
      if (!opts || (!opts.lookup && !opts.announce)) continue
      const joinDelay = Math.random() * driveList.length * 1000
      // Join with lookup: true immediately, then delay the announces.
      if (opts.lookup) this.networking.join(discoveryKey, { lookup: true })
      if (opts.announce) setTimeout(() => this.networking.join(discoveryKey, networkOpts.opts), joinDelay)
    }
  }
  ...
  async get (key, opts = {}) {
    ...
    if (opts.fuse) {
      ...
      await this.configureNetwork(drive.metadata, { lookup: true, announce: !this.noAnnounce, remember: true })
    }
    if (this.opts.stats) {
  ...
  async _configureMountNetworking (trie) {
    ...
    async function doConfiguration () {
      ...
      if (announcingNode && !lastAnnouncingNode) {
        await self.configureNetwork(feed, { remember: true, lookup: true, announce: true })
      } else if (!announcingNode && lastAnnouncingNode) {
        await self.configureNetwork(feed, { remember: true, lookup: true, announce: false })
      } else {
    ...
    async function firstConfiguration () {
      ...
      try {
        await self.configureNetwork(feed, { lookup: true, announce: false, remember: false })
      } finally {
  ...
  async configureNetwork (feed, opts = {}) {
    ...
    try {
      ...
      networkingPromise.then(continueOnce)
