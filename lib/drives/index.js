...
class DriveManager extends EventEmitter {
  ...
  async configureNetwork (opts = {}) {
    ...
    networkingPromise.catch(err => log.error({ err, key: encodedKey, opts }, 'could not configure network'))
    networkingPromise.then(() => log.debug({ key: encodedKey, opts }, 'configured network'))
