...
class DriveManager extends EventEmitter {
  ...
  async _configureMountNetworking (trie) {
    ...
    try {
      await self.configureNetwork({ key: feed.key, lookup: true, announce: false, remember: false })
    } finally {
    ...
    async function doConfiguration () {
      const announcingNode = await promisify(trie.get.bind(trie))(ANNOUNCING_FLAG_PATH, { hidden: true })
