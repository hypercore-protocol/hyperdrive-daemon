...
class DriveManager extends Nanoresource {
  ...
  async _startMirroring (drive) {
    ...
    this._mirrors.set(drive.key.toString('hex'), {
      session: mirrorSession,
      unmirror
    })
    await this._mirrorIndex.put()
  ...
  async get (key, opts = {}) {
    ...
    const mountListener = async (trie) => {
      ...
      if (mountConfig) await this.cores.configureNetwork(feed, mountConfig)
  ...
  async _rpcAllNetworkConfigurations (call) {
    const networkConfigurations = await this.network.getAllNetworkConfigurations()
