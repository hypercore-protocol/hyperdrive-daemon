...
class TopicHandler {
  ...
  _onopen (openMessage) {
    ...
    var handles = this.manager.handles.get(this._topicName)
    if (!handles) {
      handles = []
      this.manager.handles.set(this._topicName, handles)
    }
    handles.push(this)
  ...
  close () {
    ...
    var handles = this.manager.handles.get(this._topicName)
    const idx = handles.indexOf(this)
    if (idx !== -1) {
      handles.splice(idx, 1)
    }
    if (!handles.length) {
      this.manager.handles.delete(this._topicName)
      this.peersockets.close(this._topicName)
    }
  }
}
