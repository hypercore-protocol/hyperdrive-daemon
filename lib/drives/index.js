...
class DriveManager extends EventEmitter {
  constructor (corestore, networking, db, opts) {
    ...
    // TODO: Can close drives on session close because corestore does reference counting in >v5.
  ...
  async get (key, opts) {
    ...
    var checkout = null
    if (!drive) drive = hyperdrive(this.corestore, key, driveOpts)
    ...
    if (checkout) this._drives.set(keyString, checkout)
  ...
  getHandlers () {
    return {
      ...
      download: async (call) => {
        ...
        const statsInterval = call.request.getStatsinterval()
        ...
        const dl = drive.download(path, { detailed, statsInterval })
        ...
        function start (totals, fileStats) {
          const type = rpc.drive.messages.DownloadResponse.Type.START
          const rsp = toResponse(type, totals, fileStats)
          call.write(rsp)
        }

        function finish (totals, fileStats) {
          const type = rpc.drive.messages.DownloadResponse.Type.FINISH
          const rsp = toResponse(type, totals, fileStats)
          cleanup()
          call.write(rsp)
          call.end()
        ...
        function cancel (err, totals, fileStats) {
          const type = rpc.drive.messages.DownloadResponse.Type.CANCEL
          const rsp = toResponse(type, totals, fileStats)
          cleanup()
          call.write(rsp)
          call.end()
        ...
        function progress (totals, fileStats) {
          const type = rpc.drive.messages.DownloadResponse.Type.PROGRESS
          const rsp = toResponse(type, totals, fileStats)
          call.write(rsp)
        }

        function toResponse (type, totals, fileStats) {
          const rsp = new rpc.drive.messages.DownloadResponse()
          rsp.setDownloadid(downloadId)
          rsp.setType(type)

          const progress = toDownloadProgress(totals)
          rsp.setProgress(progress)

          if (detailed && fileStats) {
            const byFile = []
            for (let [name, stats] of fileStats) {
              const fileStatus = getFileStatusRsp(name, stats)
              byFile.push(fileStatus)
            }
            rsp.setFilesList(byFile)
          }
        ...
        function cleanup () {
          if (ended) return
          ended = true
          self._downloads.delete(downloadId)
      ...
      fileStats: async (call) => {
        ...
        return new Promise((resolve, reject) => {
          drive.fileStats(path, (err, stats) => {
            if (err) return reject(err)

            const rsp = new rpc.drive.messages.FileStatsResponse()
            rsp.setStats(toFileStats(stats))
