        ...
        if (op === 'open') {
          ...
          try {
            ...
            if (filename === 'networking.json') {
              ...
              stats.forEach(stat => {
                if (stat.metadata && stat.metadata.key) stat.metadata.key = datEncoding.encode(stat.metadata.key)
                if (stat.content && stat.content.key) stat.content.key = datEncoding.encode(stat.content.key)
              })
            } else {
