...
class FuseManager extends EventEmitter {
  ...
  async mount (mnt, mountOpts = {}) {
    ...
    if (this._rootDrive) {
      if (mnt.startsWith(this._rootMnt)) {
        const relativePath = mnt.slice(this._rootMnt.length)
        if (!relativePath.startsWith('/home')) throw new Error('You can only mount sub-hyperdrives within the home directory.')
        return mountSubdrive(relativePath)
      }
      await this.unmount()
    }
    return mountRoot()

    async function mountSubdrive (relativePath) {
      const drive = await self.driveManager.get(mountOpts.key, mountOpts)
    ...
    async function mountRoot () {
      const drive = await self.driveManager.get(mountOpts.key, { ...mountOpts, configure: { rootDrive: true } })
