...
class DriveManager extends EventEmitter {
  ...
  async get (key, opts = {}) {
    ...
    if (drive.writable && opts.root) {
      // Ensure that there is always a session open for the root drive so that it's not GC'd.
      ...
      await this.configureNetwork(drive, { lookup: true, announce: true, remember: true })
    }
    if (this.opts.stats) {
  ...
  async configureNetwork (drive, opts) {
    ...
    if (seeding) {
      this.networking.seed(drive.discoveryKey, opts)
    } else {
      this.networking.unseed(drive.discoveryKey)
    }
    if (opts.remember) {
      if (seeding) await this._seedIndex.put(encodedKey, { key: datEncoding.encode(drive.key) })
  ...
  getHandlers () {
    return {
      ...
      configureNetwork: async (call) => {
        ...
        const drive = this.driveForSession(id)
        const opts = {
          announce: call.request.getAnnounce(),
          lookup: call.request.getLookup(),
          remember: call.request.getRemember()
        }
