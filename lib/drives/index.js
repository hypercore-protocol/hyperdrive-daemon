...
class DriveManager extends EventEmitter {
  constructor (corestore, networking, db, opts) {
    ...
    // TODO: Can close drives on session close because corestore does reference counting in >v4.
  ...
  async get (key, opts) {
    ...
    if (!drive) {
      console.log('getting namespace for key string:', keyString)
      var namespace = await this._getNamespace(keyString)
      console.log('existing namespace is:', namespace)
    ...
    console.log('waiting for drive ready')
    ...
    console.log('key string after update:', keyString)
    ...
    console.log('drive is writable?', drive.writable)
      ...
      fileStats: async (call) => {
        ...
        return new Promise((resolve, reject) => {
          drive.stats(path, (err, stats) => {
            ...
            console.log('returning stats from server:', stats)
