...
class FuseManager extends EventEmitter {
  ...
  async _refreshMount () {
    try {
      const { mountInfo, mnt } = await this.db.get('root-drive')
      await this.mount(mnt, mountInfo)
    } catch (err) {
  ...
  async mount (mnt, mountOpts = {}) {
    ...
    const mountInfo = await hyperfuse.mount(this._rootDrive, mnt, {
      force: true,
      displayFolder: true,
...
function createFuseHandlers (fuseManager) {
  return {
    mount: async (call) => {
      const mountOpts = call.request.opts || {}
      const mnt = call.request.path
      if (!mnt) throw new Error('A mount request must specify a mountpoint.')

      const rsp = rpc.fuse.messages.MountResponse()
