...
class HyperdriveDaemon extends EventEmitter {
  ...
  async _ready () {
    ...
    const onGlobalFlush = this._registerCoreTimeouts()
    ...
    this.drives.on('global-flush', onGlobalFlush)
  ...
  _registerCoreTimeouts () {
    ...
    this.corestore.on('feed', core => {
      ...
      flushSets.set(dkeyString, { flushSet, peerAddSet })
      core.once('peer-add', () => callAllInSet(peerAddSet))

      const timeouts = {
        get: (cb) => {
          if (globalFlushes.get(dkeyString) || this.networking.flushed(discoveryKey)) return cb()
    ...
    return onGlobalFlush

    function onGlobalFlush () {
      for (const [dkeyString, { flushSet, peerAddSet }] in flushSets) {
        callAllInSet(flushSet)
        callAllInSet(peerAddSet)
        globalFlushes.set(dkeyString, true)
      }
    }
  }
