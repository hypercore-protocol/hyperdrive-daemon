...
class FuseManager extends EventEmitter {
  ...
  _wrapHandlers (handlers) {
    ...
    const NetworkInfoHandler = {
      ...
      handler: (op, match, args, cb) => {
        ...
        else if (op === 'readdir') return cb(0, ['Stats', 'Active'])
  ...
  async mount (mnt, mountOpts = {}) {
    ...
    // TODO: In order to get around tricky garbage collection issues, we need to keep a session open for any drive accessed via FUSE.
