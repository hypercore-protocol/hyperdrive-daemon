...
class FuseManager extends EventEmitter {
  ...
  async _keyForPath (path) {
    ...
    if (!path.startsWith(this._rootMnt)) throw new Error('The mountpoint must be a beneath /hyperdrive.')
    const self = this
    ...
    function getSubdriveKey(relativePath) {
      return new Promise((resolve, reject) => {
        self._rootDrive.stat(relativePath, (err, stat, trie) => {
          if (err && err.errno !== 2) return reject(err)
          else if (err) return resolve(null)
  ...
  async mount (mnt, mountOpts = {}) {
    ...
    const { drive, relativePath } = await this._driveForPath(mnt, mountOpts)
  ...
  async key (mnt) {
    await ensureFuse()
    const { key: driveKey } = await this._keyForPath(mnt)
    if (!driveKey) throw new Error(`A drive is not mounted at path: ${mnt}`)
    return datEncoding.encode(driveKey)
  }
