...
const ANNOUNCING_FLAG_PATH = '/hyperdrive-daemon/announcing'
...
class DriveManager extends EventEmitter {
  ...
  async get (key, opts = {}) {
    ...
    drive.on('mount', async (trie) => {
      ...
      log.info({ key: feed.key.toString('hex') }, 'registering mountpoint in drive index')
      await this._configureMountNetworking(trie)
      try {
  ...
  async _configureMountNetworking (trie) {
    // Check if the mount's writer has published the mount (meaning it should be swarmed independently of its parent).
    // This is not necessary if the feed is writable.
    const feed = trie.feed
    const self = this
    var wait = false

    if (feed.writable) {
      this._configuredMounts.add(feed.key.toString('hex'))
      this.emit('configured-mount', feed.key)
      return
    }
    // Block the guard immediately. This will be continued in firstConfiguration.
    feed.ifAvailable.wait()

    const lowConfigure = low(cb => callbackify(doConfiguration)(null, cb))
    let lastAnnouncingNode = null
    // Check for an earlier version of the node. If one exists, do not wait.
    await new Promise(resolve => lowConfigure(resolve))
    wait = true

    await firstConfiguration(!lastAnnouncingNode)

    feed.on('append', lowConfigure)
    this.emit('configured-mount', feed.key)
    this._configuredMounts.add(feed.key.toString('hex'))

    async function doConfiguration (opts = {}) {
      const announcingNode = await promisify(trie.get.bind(trie))(ANNOUNCING_FLAG_PATH, { wait, hidden: true })
      // Only configure the network if the announce status has changed.
      if (announcingNode && !lastAnnouncingNode) {
        await self.configureNetwork({ key: feed.key, remember: true, lookup: true, announce: true })
      } else if (!announcingNode && lastAnnouncingNode) {
        await self.configureNetwork({ key: feed.key, remember: true, lookup: true, announce: false })
      } else {
        // If both flags exist or neither exist, skip.
      }
      lastAnnouncingNode = announcingNode
    }

    async function firstConfiguration (wait) {
      // Enable lookups for the mountpoint so that the announce flag can be loaded, but don't persist this setting.
      try {
        const prom = self.configureNetwork({ key: feed.key, lookup: true, announce: false, remember: false })
        if (wait) await prom
      } finally {
  ...
  _setAnnounceFlag (drive, announce) {
    const op = announce ? drive.db.put.bind(drive.db, ANNOUNCING_FLAG_PATH, '') : drive.db.del.bind(drive.db, ANNOUNCING_FLAG_PATH)
    return new Promise((resolve, reject) => {
      op({ hidden: true }, err => {
        if (err) return reject(err)
        return resolve()
  ...
  async configureNetwork (opts = {}) {
    ...
    if (opts.drive) {
      var discoveryKey = opts.drive.discoveryKey
      var key = opts.drive.key
    } else {
      key = opts.key
      discoveryKey = opts.discoveryKey || hypercoreCrypto.discoveryKey(key)
    }
    const encodedKey = datEncoding.encode(discoveryKey)

    const sameConfig = await sameNetworkConfig(encodedKey, opts)
    ...
    if (seeding) {
      await this.networking.join(discoveryKey, networkOpts)
    } else {
      await this.networking.leave(discoveryKey)
    }
    if (opts.remember) {
      if (seeding) await this._seedIndex.put(encodedKey, { key: datEncoding.encode(key), opts: networkOpts })
    ...
    function sameNetworkConfig (discoveryKey, opts = {}) {
      return new Promise((resolve, reject) => {
        self._seedIndex.get(discoveryKey, (err, value) => {
          if (err) return resolve(false)
          const networkOpts = value.opts
          return resolve(
            networkOpts.announce === opts.announce &&
            networkOpts.lookup === opts.lookup &&
            networkOpts.remember === opts.remember
          )
        })
      })
    }
  ...
  async getNetworkConfiguration (drive) {
    ...
    try {
      ...
      log.error({ error: err }, 'could not get networking config for drive')
      return null
    }
  }
  ...
  getHandlers () {
    return {
      ...
      configureNetwork: async (call) => {
        ...
        if (drive.writable) {
          await this._setAnnounceFlag(drive, opts.announce)
        }
