...
class DriveManager extends EventEmitter {
  ...
  async _configureMountNetworking (trie) {
    ...
    const lowConfigure = low(callbackify(doConfiguration))
  ...
  async configureNetwork (opts = {}) {
    ...
    try {
      networkingPromise.catch(err => log.error({ err, key: encodedKey }, 'could not configure network'))
      networkingPromise.then(() => log.debug({ key: encodedKey }, 'configured network'))
    } catch (err) {
      log.error({ err}, 'error handling networking promises')
    }
